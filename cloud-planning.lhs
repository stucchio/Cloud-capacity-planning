...
The constraints demand that the solution `[ x[0], x[1] ]` lives in a bounded set, specifically the triangle with vertices `(0,0)`, `(4,0)` and `(0,2)`. And the goal is to make 2*x+y as big as possible.

So where can the solution lie?

The first fact to observe is that it can't lie on the *interior* of the triangle. It must be on one of the edges. To see why this is so, imagine the maximum were at some point`(x0, y0)` inside the triangle but not on the edge. In that case, I can find a very small value `d` for which `(x0+d, y)` is still inside the triangle - specifically, I just pick `d` equal to half the distance to the edge. If I compute `f(x0+d,y) = 2*x0+2*d+y`, then this value is bigger than `f(x0,y)=2*x0+y`. This means that `(x0, y0)` was not really the maximum.
...
>     reservationFixedCostsObj = [ ( cost / 365.0, "reservation_" ++ kind) | (kind, cost) <- reservationFixedCosts ] -- cost of reserving an instance
>     reservationVariableCostsObj = [ (cost*8, "reserved_" ++ kind ++ "_" ++ period) -- cost of *running* the reserved instance
>                                  | (kind, cost) <- reservationVariableCosts,
>                                  period <- periods ]
>     onDemandVariableCostsObj = [ (onDemandHourlyCost*8, "onDemand_" ++ p) | p <- periods ] -- cost of running on-demand instances
>                         in
...
> capacityConstraints loadPattern = [ (linCombination ([ (1.0, "onDemand_" ++ period) ] ++ [(1.0, "reserved_" ++ k ++ "_" ++ period) | k <- reservationTypes]), load)
